...
router.post("/", validateUser(userSchema), async (req, res) => {
  try {
    ...
    if (cpf) {
      const existingCpf = await getQuery("SELECT id FROM users WHERE cpf = ?", [
        cpf,
      ]);
      if (existingCpf) {
...
router.put("/:id", validateUser(updateUserSchema), async (req, res) => {
  try {
    ...
    const existingUser = await getQuery("SELECT id FROM users WHERE id = ?", [
      id,
    ]);
    if (!existingUser) {
      return res.status(404).json({ error: "Usuário não encontrado" });
    ...
    if (email) {
      ...
      if (emailExists) {
        return res
          .status(409)
          .json({ error: "Email já cadastrado para outro usuário" });
      }
    }

    if (cpf) {
      const cpfExists = await getQuery(
